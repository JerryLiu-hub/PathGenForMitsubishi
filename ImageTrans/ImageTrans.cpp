 ...
 */
void LineSolver(std::vector<std::pair<cv::Point2f, cv::Point2f>>& lines, cv::InputArray item,
	const line_type type, const float threshold_min, const float threshold_max, // 阈值下限和上限
	const float mininum_line_length,
	const float sin_theta, const float cos_theta)
{
	cv::Mat plot = item.getMat();
	...
	case UPDOWN:
		for (shift = drawstep; shift < 1; shift += drawstep) {
			checkpoint1 = 0;
			for (y = drawstep; y < 1; y += drawstep) {
				if (coordinate_mapping_pic2std(item.getMat(), y, shift) > threshold_max) {
					if (!checkpoint1) { // record the entry point
						tmp_line.first.x = shift;
						tmp_line.first.y = y;
					}
				...
				else if (coordinate_mapping_pic2std(item.getMat(), y, shift) < threshold_min) {
					if (checkpoint1) { // record the end point
						tmp_line.second.x = shift;
						tmp_line.second.y = y;
						distance = fabsf(tmp_line.second.y - tmp_line.first.y);
	...
	case LEFTRIGHT:
		for (shift = drawstep; shift < 1; shift += drawstep) {
			checkpoint1 = 0;
			for (x = drawstep; x < 1; x += drawstep) {
				if (coordinate_mapping_pic2std(item.getMat(), shift, x) > threshold_max) {
					if (!checkpoint1) {
						tmp_line.first.x = x;
						tmp_line.first.y = shift;
					}
				...
				else if (coordinate_mapping_pic2std(item.getMat(), shift, x) < threshold_min) {
					if (checkpoint1) {
						tmp_line.second.x = x;
						tmp_line.second.y = shift;
						distance = fabs(tmp_line.second.x - tmp_line.first.x);
	...
	case FORWARDSLASH: // 正交斜线使用两轮遍历法进行绘制
		...
		for (shift = drawstep; shift < 1; shift += drawstep) { // 行遍历
			checkpoint1 = 0;
			for (x = shift, y = drawstep; (x < 1) && (x > 0) && (y < 1) && (y > 0); x += cos_theta * drawstep, y += sin_theta * drawstep) {
				if (coordinate_mapping_pic2std(item.getMat(), y, x) > threshold_max) {
					if (!checkpoint1) {
						tmp_line.first.x = x;
						tmp_line.first.y = y;
					}
				...
				else if (coordinate_mapping_pic2std(item.getMat(), y, x) < threshold_min) {
					if (checkpoint1) {
						tmp_line.second.x = x;
						tmp_line.second.y = y;
						distance = sqrt((tmp_line.second.x - tmp_line.first.x) * (tmp_line.second.x - tmp_line.first.x)
...
int main(int argc, char* argv[])
{
	...
	constexpr float binarization_threshold_min = 0.5f;
	std::vector<std::pair<cv::Point2f, cv::Point2f>> lines; // 生成的轨迹
	std::vector<std::pair<cv::Point2f, cv::Point2f>> lines_schmidt;
	...
	for (int i = 0; i < gamma_level; i++) {
		...
		LineSolver(lines_schmidt, gamma_result_level[i], static_cast<line_type>(i),
			binarization_threshold_min, binarization_threshold, min_line_length, sin(pi() / 4), cos(pi() / 4));
	...
	DrawLinePicture(hdc, lines_schmidt,
		static_cast<int>(parper_coordinate_x), static_cast<int>(parper_coordinate_y));
	cv::Mat result_bmp2 = cv::imread("DrawResult.bmp");
	show_picture(result_bmp2, "result2");
